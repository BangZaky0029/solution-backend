  ...
  class WhatsAppClient {
     ...
     */
    async initialize() {
  // Jika client sudah ada, hentikan
      if (this.client) {
      ...
      try {
        ...
        // Setup event handler dengan safety null check
    ...
    setupEventHandlers() {
      ...
      this.client.on('auth_failure', (msg) => {
        if (!this.client) return; // safety check
        if (!this.isReady) return; // opsional, hanya handle saat ready
    ...
    */
    broadcastStatus(data) {
      if (!this.io) this.io = getIO(); // safety fallback
